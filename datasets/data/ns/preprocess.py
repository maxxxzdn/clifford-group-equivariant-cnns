    ...
    for filename in tqdm(os.listdir(raw_dir)):
        if filename.endswith('.h5'):
            ...
            with h5py.File(file_path, 'r') as f:
                ...
                partition = 'train' if 'train' in filename else 'valid' if 'valid' in filename else 'test'
                ...
                data_tensor = data_tensor.reshape(B, T, C, 64, 64).permute(0, 1, 3, 4, 2)
                
                # Save each element of batch as a separate file
                for i in range(data_tensor.shape[0]):
                    output_filename = f'{counters[partition]}.pt'
                    output_path = os.path.join(output_base_dir, partition, output_filename)
