...
def generate_kernel_grid(kernel_size, dim):
    ...
    return grid.reshape(-1, dim) / max(kernel_size // 2, 1.)
...
class CliffordSteerableKernel(nn.Module):
    ...
    def setup(self):
        self.rel_pos = generate_kernel_grid(self.kernel_size, self.algebra.dim)[:, jnp.newaxis, :]
    ...
    def __call__(self):
        ...
        """        
        # Weighted Cayley
        weighted_cayley = WeightedCayley(self.algebra, self.c_in, self.c_out, self.product_paths_sum)()
        
        # Compute scalars
        scalar = compute_scalar_shell(self.algebra, self.rel_pos, self.rel_pos_sigma)

        # Embed scalar and vector into a multivector
        x = self.algebra.embed_grade(scalar, 0) + self.algebra.embed_grade(self.rel_pos, 1)
        
        # Evaluate kernel network
        k = KernelNetwork(self.algebra, self.c_in, self.c_out, self.num_layers, self.hidden_dim, self.bias_dims, self.product_paths_sum)(x)
        
        # Reshape to kernel mask
        k = k.reshape(-1, self.c_out, self.c_in, self.algebra.n_blades)
        
        # Compute kernel mask
        shell = ScalarShell(self.algebra, self.c_in, self.c_out)(self.rel_pos).reshape(-1, self.c_out, self.c_in, 2 ** self.algebra.dim)
        ...
        K = K.reshape(self.c_out * self.algebra.n_blades, self.c_in * self.algebra.n_blades, * (self.algebra.dim * [self.kernel_size]))
